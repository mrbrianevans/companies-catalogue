...
export async function handleStreamRequest(
  path: string,
  timepointInputString: string | null,
  ...
  try {
    ...
    if (!timepointInputString) return makeError(400, "Missing timepoint parameter");
    const timepoint = Number(timepointInputString);
    if (isNaN(timepoint)) return makeError(400, "Invalid timepoint parameter");

    const validRange = await getMinMaxRange(path);
    if (!validRange) return makeError(501, "No data available for " + path);

    if (timepoint > validRange.max) return makeError(416, "Timepoint out range (too big)");
    if (timepoint < validRange.min) return makeError(416, "Timepoint out range (too small)");
    ...
    const headers = new Headers({
      ...
      "access-control-allow-headers":
        "origin, content-type, content-length, user-agent, host, accept, authorization",
...
function seekInFile(filename: string, timepoint: number): Readable {
  return streamWholeFile(filename)
    .on("error", (err) => {
      console.log("whole file error", err);
    })
    .pipe(split2())
    .filter((line) => {
...
async function streamFilesToPassThrough(
