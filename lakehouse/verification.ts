...
async function main(streamPath: string) {
    ...
    console.log('Row count', lakehouseTimepoints.count, 'Distinct timepoints', lakehouseTimepoints.distinct_timepoints, 'Timepoint range', lakehouseTimepoints.diff_timepoints)
    if(lakehouseTimepoints.diff_timepoints !== lakehouseTimepoints.distinct_timepoints) {
        throw new Error('Timepoints in lakehouse are not contiguous. Min-max Diff does not match distinct timepoints.')
    }
    if(lakehouseTimepoints.count !== lakehouseTimepoints.distinct_timepoints) {
        throw new Error('Timepoints in lakehouse are not contiguous. Count does not match distinct timepoints')
    }
